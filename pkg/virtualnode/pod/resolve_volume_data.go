...
func (m *Manager) resolveVolumeData(pod *corev1.Pod) (volumeData map[string]*gopb.NamedData, err error) {
	volumeData = make(map[string]*gopb.NamedData)
	...
	for _, vol := range pod.Spec.Volumes {
		...
		case vol.ConfigMap != nil:
			...
			namedData := &gopb.NamedData{DataMap: make(map[string][]byte)}
