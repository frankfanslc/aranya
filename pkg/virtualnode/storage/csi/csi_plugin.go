...
//
//var NodeInfoManager nodeinfomanager.Interface
//
...
func (h *RegistrationHandler) RegisterPlugin(pluginName string, endpoint string, versions []string) error {
	...
	h.CSIDrivers.Set(pluginName, Driver{
		endpoint:                endpoint,
		highestSupportedVersion: highestSupportedVersion,
	})

	// Get node info from the driver.
	csi, err := newCsiDriverClient(csiDriverName(pluginName), h.CSIDrivers)
	if err != nil {
		return err
	}
	...
	err = h.NodeInfoManager.InstallCSIDriver(pluginName, driverNodeID, maxVolumePerNode, accessibleTopology)
	if err != nil {
		if unregErr := h.unregisterDriver(pluginName); unregErr != nil {
			//klog.Error(log("registrationHandler.RegisterPlugin failed to unregister plugin due to previous error: %v", unregErr))
		}
...
func (h *RegistrationHandler) validateVersions(callerName, pluginName string, endpoint string, versions []string) (*utilversion.Version, error) {
	...
	existingDriver, driverExists := h.CSIDrivers.Get(pluginName)
	if driverExists {
...
func (p *csiPlugin) Init(host volume.VolumeHost) error {
	...
	if csiClient == nil {
		...
		// set CSIDriverLister
		h, ok := host.(VolumeHost)
		if ok {
			p.csiDriverLister = h.CSIDriverLister()
	...
	//	}
	//}
	if err := p.initializeCSINode(host); err != nil {
		return errors.New(log("failed to initialize CSINodeInfo: %v", err))
	}
...
func (p *csiPlugin) initializeCSINode(host volume.VolumeHost) error {
	...
	go func() {
		...
		err := wait.ExponentialBackoff(initBackoff, func() (bool, error) {
			...
			err := p.nim.InitializeCSINodeWithAnnotation()
			if err != nil {
...
func (p *csiPlugin) NewMounter(
	...
	mounter.csiClientGetter.csiDrivers = p.csiDrivers
	...
	mounter.MetricsProvider = NewMetricsCsi(volumeHandle, dir, csiDriverName(driverName), p.csiDrivers)
...
func (p *csiPlugin) NewUnmounter(specName string, podUID types.UID) (volume.Unmounter, error) {
	...
	unmounter.csiClientGetter.csiDrivers = p.csiDrivers
...
func (p *csiPlugin) NewBlockVolumeMapper(spec *volume.Spec, podRef *api.Pod, opts volume.VolumeOptions) (volume.BlockVolumeMapper, error) {
	...
	mapper.csiClientGetter.csiDrivers = p.csiDrivers
...
func (p *csiPlugin) NewBlockVolumeUnmapper(volName string, podUID types.UID) (volume.BlockVolumeUnmapper, error) {
	...
	unmapper.csiClientGetter.csiDrivers = p.csiDrivers
...
func (h *RegistrationHandler) unregisterDriver(driverName string) error {
	h.CSIDrivers.Delete(driverName)

	if err := h.NodeInfoManager.UninstallCSIDriver(driverName); err != nil {
