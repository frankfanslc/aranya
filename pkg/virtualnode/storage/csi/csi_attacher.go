...
func (c *csiAttacher) Attach(spec *volume.Spec, nodeName types.NodeName) (string, error) {
	...
	_, err = c.k8s.StorageV1().VolumeAttachments().Create(context.TODO(), attachment, meta.CreateOptions{})
	alreadyExist := false
	if err != nil {
...
func (c *csiAttacher) MountDevice(spec *volume.Spec, devicePath string, deviceMountPath string) (err error) {
	...
	if c.csiClient == nil {
		c.csiClient, err = newCsiDriverClient(csiDriverName(csiSource.Driver), c.csiDrivers)
		if err != nil {
...
func (c *csiAttacher) Detach(volumeName string, nodeName types.NodeName) error {
	...
	if err := c.k8s.StorageV1().VolumeAttachments().Delete(context.TODO(), attachID, meta.DeleteOptions{}); err != nil {
...
func (c *csiAttacher) UnmountDevice(deviceMountPath string) error {
	...
	if c.csiClient == nil {
		c.csiClient, err = newCsiDriverClient(csiDriverName(driverName), c.csiDrivers)
		if err != nil {
